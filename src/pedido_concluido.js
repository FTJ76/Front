...
const getArgsPageLoad = () => {
  ...
  document.getElementById('lbl_id_pedido').innerHTML = 'Pedido finalizado com sucesso nº ' + id_pedido;
  return id_pedido;
}
...
const getListaConcluidos = async (id_pedido) => {

  var session_id = getSessao();
  if (session_id != null)
  {
    let url = 'http://127.0.0.1:5000/listar_pedido_concluido?session_id=' + session_id + '&pedido_id=' + id_pedido;
    fetch(url, {
      method: 'get',
    })
    .then((response) => response.json())
